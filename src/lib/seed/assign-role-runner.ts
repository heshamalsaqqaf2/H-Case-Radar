...
const YOUR_EMAIL = "heshamdev2@gmail.com"; // ⬅️ هذا إيميلك الصحيح

async function main() {
  try {
    if (!YOUR_EMAIL || YOUR_EMAIL !== "heshamdev2@gmail.com") {
      // ⬅️ غير الشرط هنا
      console.error("❌ Please set YOUR_EMAIL in assign-role-runner.ts");
      console.log(
        "📝 Open the file and change 'your-email@example.com' to your actual email",
      );
      process.exit(1);
    }
